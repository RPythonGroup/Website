...
#### Window
Window 운영체제의 경우 기본 설치폴더는 R 버전이 x.y.z이면 default로 `C:\\Program Files\\R\\R-x.y.z`로 되지만 연구회에서는 아래와 같이 추천합니다.
...
MacOS 운영체제의 경우 기본 설치폴더는 R 버전이 x.y.z이면 default로 `/Library/R.framework/Versions/x.y.z/Resource/bin/R`로 되며, 연구회에서는 기본값을 추천합니다.
